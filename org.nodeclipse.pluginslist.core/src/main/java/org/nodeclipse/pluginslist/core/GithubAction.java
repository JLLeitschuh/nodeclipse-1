...
public class GithubAction implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		...
		try {
			browser = WorkbenchBrowserSupport.getInstance().createBrowser(style,
					Constants.GITHUB_BROWSER_ID, Constants.GITHUB_BROWSER_NAME, Constants.GITHUB_BROWSER_TOOLTIP);
		} catch (PartInitException e) {
			e.printStackTrace();
			MessageDialog.openError(
				window.getShell(),
				Constants.ERROR1_MESSAGE,
				Constants.ERROR1_MESSAGE+" to show "+Constants.GITHUB_BROWSER_NAME);
			return;
		}
	    try {
			browser.openURL(Constants.githubUrl);
		} catch (PartInitException e) {
			e.printStackTrace();
		}
